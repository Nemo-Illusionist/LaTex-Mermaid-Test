...
Проект настроен для автоматической сборки через GitHub Actions. Workflow автоматически:
- Устанавливает все зависимости (TeX Live, Node.js, Mermaid CLI)
- Компилирует LaTeX документ с поддержкой русского языка
- Генерирует Mermaid диаграммы в SVG и PDF форматах
- Сохраняет артефакты для скачивания

### Запуск сборки

**Автоматически:**
- При каждом push в ветки `master` или `main`
- При создании Pull Request

**Вручную:**
1. Перейдите в раздел Actions вашего репозитория
2. Выберите workflow "Build LaTeX PDF with Mermaid"
3. Нажмите "Run workflow"

### Скачивание результатов

После успешной сборки артефакты доступны в разделе Actions:
- `main-pdf` - скомпилированный PDF документ
- `mermaid-svg-images` - диаграммы в формате SVG
- `mermaid-pdf-images` - диаграммы в формате PDF
- `build-logs` - логи компиляции (если была ошибка)

### Создание релиза

Для создания релиза с PDF:
```bash
git tag v1.0.0
git push origin v1.0.0
```

Workflow автоматически создаст GitHub Release с PDF и SVG файлами.
